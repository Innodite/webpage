<footer>
	<section>
     	<div class="ContenedorColumnas">


			<article class="columnas columna1"	>
				<h3>Enlaces de Interés</h3>	
				<section id="menu-inferior">
					<ul>
						<li><a href="http://www.madboxpc.com/">Mad Box Pc</a></li>
						<li><a href="http://www.razorman.net/">RazorMan</a></li>
						<li><a href="http://www.xda-developers.com/">XDA Developers</a></li>
						<li><a href="http://www.hdmagazine.org/">Hackers & Developers</a></li>					 
						<li><a href="http://www.linuxjournal.com/">Linux Journal</a></li>
					</ul>
				</section>											
			</article>

					
			<article class="columnas columna1" >                                                                
                <div class="social-icons-row">
	                 <a href="servicios.php"><img src="../imagenes/servicios.png" style="width: 50px; height: 50px"></a> <br>
	                    <h3>Servicios<h3><br>  
	            </div> 
                <div class="social-icons-row">
	                <a href="portafolio.php"><img src="../imagenes/trabajos2.png" style="width: 70px; height: 70px"></a> <br>
	                <h3>Trabajos Realizados</h3>
                </div><br>                                                                                                                
			</article>


			<article class="columnas columna1" >
                <div class="social-icons-row">
	               <a href="#"><img src="../imagenes/telefono.png" style="width: 50px; height: 50px"></a> <br>
	                    <h3>0416-583.38.09</h3>
                        <h3>0426-594.00.45</h3>
                </div><br>
                <div class="social-icons-row">
	                <a href="#"><img src="../imagenes/correo.png" style="width: 50px; height: 50px"></a> <br>
	                    <h3>contacto@innodite.com<h3><br>                                            		
                </div>
			</article>
					

			<article class="columnas columna1" >	
				<h3>Título Noticia</h3>											
				<p id="ver-mas">Es un hecho establecido hace demasiado tiempo que un lector se distraerá con el contenido del texto de un sitio mientras que mira su diseño. El punto de usar Lorem Ipsum es que tiene una distribución más o menos normal de las letras, al contrario de usar textos como <a href="#" class="submit"> Leer Mas</a></p>
			</article>


						
		</div>			
		<p class="pie">Copyright © 2014 Innovación y Diseño C.A, J-40270256-6</p>
	</section>
</footer>